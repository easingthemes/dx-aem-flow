...
keyPoints:
  ...
  - "**Step 2 — Verify** (/dx-bug-verify) — Opens Playwright MCP, navigates to the repro URL, follows repro steps (click, fill, navigate), takes screenshots at each step, and confirms whether the bug reproduces."
  ...
  - "**Visual verification matters** — 'The hero component overlaps the nav on mobile' can't be verified by reading code. Playwright MCP makes the AI see what the user sees."
actionItems:
  - |
  ...
  - |
    **Prerequisites**
    - Playwright MCP must be configured for the visual verify and fix steps
