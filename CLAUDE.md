...
- **dx-automation** (`plugins/dx-automation/`) — Autonomous AI agents (DoR checker, DoD checker, DoD fixer, PR reviewer, PR answerer, BugFix agent, QA agent, DevAgent, DOCAgent, Estimation, SimpleAgent) running 24/7 as ADO pipelines triggered by AWS Lambda webhooks. Requires dx. Skills prefixed `auto-*`.
  - **SimpleAgent** (tag `KAI-SIMPLE-AUTOMATION`) applies small AEM changes (a11y / color / spacing / copy) by splitting work into authoring (AEM MCP write) or code (file edits → PR) paths. 9 confidence gates; ≤5 files / ≤50 lines / ≤10 JCR writes. Backed by the `/dx-simple` skill in dx-core. **Resumable recovery (TODO #141):** the per-ticket git branch is a durable state store — every phase/gate checkpoints `resume-state.json` (via `scripts/save-state.sh`); Phase 0 (`scripts/resume-check.sh`) discovers the branch by anchored ticket-id match and dispatches on `status`. A blocked/crashed run resumes when a human replies on the ticket with the `@kai-simple` keyword — an **Azure-native filtered Service Hook** re-runs the pipeline (the Lambda `wi-router.mjs` is **not** in this path). Token + attempt cap are config (`dx-simple.recovery.*`); requires `.ai/specs/` tracked in git; recovery is **human-re-triggered, never automatic**.
