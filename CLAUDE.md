...
- **dx-automation** (`plugins/dx-automation/`) — Autonomous AI agents (DoR checker, DoD checker, DoD fixer, PR reviewer, PR answerer, BugFix agent, QA agent, DevAgent, DOCAgent, Estimation, SimpleAgent) running 24/7 as ADO pipelines. Most are triggered by AWS Lambda webhooks; **SimpleAgent and BugFix are triggered by an Azure-native Service Hook (no Lambda)** — a Bug comment containing `@kai-bugfix` fires the BugFix pipeline directly, mirroring SimpleAgent's `@kai-simple`. BugFix (`/dx-bug-all`) has full resumable recovery over its triage→verify→fix steps (the per-ticket `bugfix/<id>-*` branch is the durable state store; `dx-bug-all.recovery.*` config). Requires dx. Skills prefixed `auto-*`.
