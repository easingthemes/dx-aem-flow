...
when_to_use: "Use when importing requirements from a markdown file or local spec instead of an ADO/Jira ticket. Trigger on 'import requirements', 'requirements from file', 'dx-req-import', or when there is no ticket to fetch. Do NOT use when an ADO/Jira ticket exists (that's dx-req), to create child Tasks (dx-req-tasks), or to check Definition of Done (dx-req-dod)."
