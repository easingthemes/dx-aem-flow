...
when_to_use: "Use after /dx-figma-prototype to verify it matches the Figma reference. Trigger on 'verify prototype', 'compare prototype', 'figma verify', 'check prototype against figma', or 'does the prototype match'. Do NOT use to verify a component on a running AEM instance (that's aem-fe-verify) or to run pre-PR code checks (dx-step-verify)."
