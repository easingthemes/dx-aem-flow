...
**ADO fast path:** the script always re-fetches and overwrites
`raw-workitem.json` + `raw-story.md`. Pre-seeded files (e.g. from
`/dx-hub-dispatch`) get respected only if you skip step 2 entirely — check for
the pre-seeded `raw-story.md` BEFORE invoking the script and bypass the
