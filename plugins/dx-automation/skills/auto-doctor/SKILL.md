...
WI Router required keys: `ADO_PAT`, `ADO_ORG_URL`, `BASIC_USER`, `BASIC_PASS`, `WEBHOOK_SECRET`, `DYNAMODB_DEDUPE_TABLE`, `DYNAMODB_RATE_LIMIT_TABLE`, `SQS_DLQ_URL`, `ADO_DOR_PIPELINE_ID`, `ADO_DOD_PIPELINE_ID`, `ADO_QA_PIPELINE_ID`, `ADO_DEV_PIPELINE_ID`, `ADO_DOC_PIPELINE_ID`, `ADO_ESTIMATION_PIPELINE_ID`, `TAG_GATE_DOR`, `TAG_GATE_DOD`, `TAG_GATE_QA`, `TAG_GATE_DEV`, `TAG_GATE_DOC`, `TAG_GATE_ESTIMATION`. (BugFix is **not** WI-Router-routed — it uses an Azure-native `@kai-bugfix` comment hook like SimpleAgent — so `ADO_BUGFIX_PIPELINE_ID` / `TAG_GATE_BUGFIX` are **not** required here.)
