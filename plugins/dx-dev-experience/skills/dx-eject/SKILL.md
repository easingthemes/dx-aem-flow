...
- `dx` — eject dx-dev-experience only
...
- dx-dev-experience (45 skills, 5 agents, 4 rules, shared files, data, templates)
...
### After ejecting:
- You can uninstall the plugins (`/plugin uninstall dx-dev-experience`, etc.)
...
│   ├── dx-dev-experience/  # lib/, templates/, hooks/
...
│   ├── dx-dev-experience/  # rules/, skills/, agents/, docs/, instructions/
│   ├── dx-aem/             # rules/, agents/, instructions/
│   └── dx-automation/      # rules/
├── hooks/            # Plugin hooks.json files
│   └── dx-dev-experience/hooks.json
└── manifests/        # Plugin version snapshots
    ├── dx-dev-experience.json
    ├── dx-aem.json
    └── dx-automation.json
```
...
Read `.ai/ejected/hooks/dx-dev-experience/hooks.json` (if it exists). This contains the Stop hook that runs `stop-guard.sh`.
...
- **dx-dev-experience:** Empty mcpServers (ADO MCP configured separately) — no merge needed
...
2. **Uninstall plugins:** `/plugin uninstall dx-dev-experience`, `/plugin uninstall dx-aem`, `/plugin uninstall dx-automation`
...
2. `/dx-eject dx` — Ejects only dx-dev-experience. Copies 45 skills and 5 agents. AEM and automation plugins remain as plugins.

3. `/dx-eject aem` — Ejects only dx-aem. Copies 10 AEM skills and 5 AEM agents. dx-dev-experience stays as a plugin (AEM skills depend on dx skills, which still load from the plugin).
