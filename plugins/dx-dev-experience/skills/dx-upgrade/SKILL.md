...
2. `/dx-upgrade dx` — Scopes the upgrade to only the dx-dev-experience plugin. Updates `audit.sh` and `stop-guard.sh` from plugin data, installs 4 new Copilot skill templates, and reports "Suggest re-run: `/dx-doctor` to verify."
...
```bash
/plugin uninstall dx-dev-experience
