...
- `--config .ai/playwright/config.json` — **consumer-generated, gitignored** (carries `httpCredentials` from `QA_BASIC_AUTH_*` + viewport). Generated by `aem-init`; absent-file is tolerated (publisher auth simply unset).
...
| chrome-devtools `X` | playwright `Y` | Notes |
|---------------------|----------------|-------|
| `navigate_page` | `browser_navigate` | |
| `take_screenshot` | `browser_take_screenshot` | |
| `take_snapshot` | `browser_snapshot` | a11y tree + element refs (same UID model) |
| `evaluate_script` | `browser_evaluate` | + `browser_run_code_unsafe` for full Playwright API |
| `click` | `browser_click` | |
| `fill` | `browser_type` | single field |
| `fill_form` | `browser_fill_form` | |
| `type_text` | `browser_type` | |
| `press_key` | `browser_press_key` | |
| `hover` | `browser_hover` | |
| `drag` | `browser_drag` (+ `browser_drop`) | |
...
| `get_network_request` | `browser_network_request` | |
| `wait_for` | `browser_wait_for` | |
| `resize_page` | `browser_resize` | |
| `list_pages` | `browser_tabs` (list) | merged |
| `select_page` | `browser_tabs` (select) | merged |
| `new_page` | `browser_tabs` (new) | merged |
| `close_page` | `browser_tabs` (close) / `browser_close` | merged |
| `emulate` | `--device` / `--viewport-size` (launch) | no runtime tool; `browser_resize` for viewport |
| `lighthouse_audit`, `performance_*`, `take_heapsnapshot` | — REMOVED | only used by `dx-perf` (being removed) |
