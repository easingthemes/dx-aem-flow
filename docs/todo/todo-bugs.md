...
**Upstream check (2026-09-19) — UNBLOCKED, fix is ours to make.** Read the tool's source at `@azure-devops/mcp` **v2.10.0** (`src/tools/work-items.ts`, `wit_work_item_attachment`):
- **No size cap in the tool.** It concatenates the whole stream into a Buffer and base64-encodes it unconditionally. So the truncation is *not* in this tool's logic — it is the JSON-RPC/transport boundary, which matches the byte-identical-across-two-paths evidence. Filing upstream stays worthwhile but will not be a quick fix, and there is still no issue for it there.
- **The tool now accepts `savePath`.** When given a relative directory, it does `fs.writeFileSync(path.join(savePath, fileName), buffer)` and returns `Attachment saved to: <path>` — **no base64 crosses the wire at all**, so the truncation cannot occur. Constraints: `savePath` must be relative (absolute paths and `..` are rejected) and resolves against the MCP server's cwd; it throws if the target file already exists.
- **Our code does not use it yet:** `plugins/dx-core/data/lib/fetch-raw-story.js` calls `wit_work_item_attachment` with `{project, attachmentId, fileName}` only, taking the base64 resource path.

**Revised approach:** pass `savePath` (e.g. the spec dir's `images/`) in `fetch-raw-story.js`, drop the base64 decode for that path, keep `validate-image.sh` as the guard, and keep the REST+PAT fallback idea only for older MCP versions. See [2026-09-19-upstream-dependency-check.md](../research/2026-09-19-upstream-dependency-check.md).
...
## dx-step-all ran one step per invocation

**Added:** 2026-09-24
**Problem:** `/dx-step-all` executed one step, printed that step's summary, and ended. It wrote no `runs.jsonl` record. `dx-step` had no `context: fork`, so `Skill(dx-step)` loaded dx-step's whole SKILL.md inline into the coordinator. The coordinator then followed dx-step's own ending: "execute exactly one step, then stop" and "Run `/dx-step-all` to continue". It never returned to its graph, so it reached neither exit and never logged. The earlier "partial" runs (6/7, 10/11) had the same failure. They were finished by the parent session sending "keep going", after which the coordinator implemented the remaining steps itself. A headless spike on CLI 2.1.281 reproduced it: an inline worker stopped a 3-iteration coordinator after one iteration in 1 of 2 runs, and a forked worker completed 3 of 3 runs. That rules out a regression. The skill text was byte-identical from v3.6.7 to v3.7.1.
**Scope:** `plugins/dx-core/skills/{dx-step,dx-step-fix,dx-step-all,dx-bug-fix,dx-agent-all}/SKILL.md`, `plugins/dx-core/shared/orchestration-check.md` (§ Step-loop marker), `website/src/pages/learn/_dx-workflow/script.js`.
**Done-when:** `grep -l '^context: fork' plugins/dx-core/skills/dx-step{,-fix}/SKILL.md` lists both, **and** one `/dx-step-all` run in a consumer project with ≥3 pending steps completes them all without a nudge. Its transcript must show a separate subagent per `dx-step` call, and a `"flow":"step-all"` line must land in `.ai/learning/raw/runs.jsonl`.
**Approach:** Worker skills are forked, end with `## Return`, and suppress their human-facing "run X next" hints while `.ai/run-context/step-loop.flag` exists. The coordinator gets a loop invariant (two exits only, a per-iteration `Loop: <P> pending` assertion, a no-progress cap) and branches on Return verdicts. A frontmatter Stop hook guard was considered and rejected. Skill-frontmatter hooks stay registered for the rest of the session. Stop→SubagentStop conversion is documented for agents only. Block decisions have been dropped in some configurations (anthropics/claude-code#86569, #20221). Related: #218 (`plan-metadata.sh` heading fragility; the 2026-09-16 run hit it).
