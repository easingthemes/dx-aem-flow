...
| 81 | dx-rfp: pre-run cost visibility (transparency, not gating) | Low | Open | 2026-04-16 | [todo-dx-rfp.md#pre-run-cost-visibility-keep--not-gating-just-transparency](todo-dx-rfp.md#pre-run-cost-visibility-keep--not-gating-just-transparency) — one-line agent-count estimate; no budget ceiling, no model downgrade |
