...
| # | Decision | Choice |
|---|----------|--------|
| 1 | Recovery input channel | **ADO comment containing `@kai-simple`.** The trigger is an **Azure-native Service Hook** on the *"work item commented on"* event, **with a subscription filter requiring the comment body to contain `@kai-simple`** → service connection → pipeline re-runs. This path **does not go through the Lambda** (`wi-router.mjs`) at all — the SimpleAgent trigger (first run **and** recovery, both via the `@kai-simple` comment) is configured in ADO, not in the Lambda router. The pipeline receives `workItemId` from the hook resource mapping; the agent **reads the comment itself** from the work item. No router change. |
...
2. Strip the token; the remainder is `continue-input`. Apply it to `blocked-at-phase` (e.g. set `simple-block.yaml` `element` for a G1 ambiguity; add a file/line/anchor hint for G4). **Commit the updated `comment-cursor` before / atomically with re-entering the phase (L2)** — otherwise a crash after applying the answer reprocesses the same comment next run.
