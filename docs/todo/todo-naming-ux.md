...
**Upstream check (2026-09-19): UNBLOCKED — upstream fixed.** Live probe on Claude Code **v2.1.277**: a throwaway plugin's skills registered as `testns:hello` (with `name:` in frontmatter) and `testns:world` (without), and our own as `dx-core:dx-init`. Plugin skills are namespaced `plugin:skill` regardless of `name:` — the defect this workaround existed for. The fix shipped in v2.1.216; the feature request we tracked ([#50486](https://github.com/anthropics/claude-code/issues/50486)) and the bug report ([#22063](https://github.com/anthropics/claude-code/issues/22063)) were both stale-closed as *not planned* after the behavior already landed. GH #24 closed. **This is now a decision, not a blocker:** the prefixes stutter (`dx-core:dx-init`), but renaming touches 78 skills, every cross-reference, both catalogs, the website, the scaffold CLI and six months of users' muscle memory around `/dx-init`. Pick one — rename, or write the prefix down as a deliberate convention and close this item. See [2026-09-19-upstream-dependency-check.md](../research/2026-09-19-upstream-dependency-check.md).
