...
**Scope:** `plugins/dx-core/skills/dx-simple/SKILL.md` (scope classification — Phase 0/1), `plugins/dx-core/skills/dx-simple/scripts/parse-simple-block.sh` (parse a `scope`/`repo` field), `plugins/dx-core/skills/dx-simple/templates/simple-block.md.tmpl` (surface the field in the DoR block), `plugins/dx-automation/data/pipelines/cli/ado-cli-simple.yml` (+ a new thin router pipeline YAML, e.g. `ado-cli-simple-router.yml`), `plugins/dx-automation/skills/auto-webhooks/` (point the Service Hook at the router), and config schema in `docs/reference/` (`repos[]` role → dx-simple pipeline-id map, reusing the existing `CROSS_REPO_PIPELINE_MAP` pattern).
**Done-when:** A two-repo (FE + BE) project can post a single `@kai-simple` comment and have **only** the declared repo(s)' dx-simple pipeline run. Verify: (1) `grep -n 'scope\|repo' plugins/dx-core/skills/dx-simple/scripts/parse-simple-block.sh` shows the simple block parses a fe/be/both scope field; (2) a router exists (new `ado-cli-simple-router.yml` **or** a documented router-repo pipeline) that reads a `repos[]`→pipeline-id map and queues child pipeline runs via the ADO REST API passing `workItemId`; (3) a dry-run with `scope: be` queues the BE pipeline and **not** the FE pipeline.
**Approach:** Keep the router **thin and stateless** — this is the legitimate "one logical project, two repos" case, **not** a revival of the dead central-hub model (cross-ref #133: don't build a stateful cross-project brain). Concretely:
  ...
  - Router classifies scope: prefer an explicit `scope: fe|be|both` (or `repo:`) field in the `simple` DoR block (deterministic, cheapest); fall back to a lightweight classifier that maps the component path / file list to `repos[].role`.
  - Router looks up each target in a `repos[]` role → dx-simple-pipeline-id map (reuse the existing `CROSS_REPO_PIPELINE_MAP` env convention) and **queues the child pipeline run(s) via the ADO REST API** (`Runs - Run Pipeline`, passing `workItemId`). Then it **exits** — no waiting, no aggregation.
