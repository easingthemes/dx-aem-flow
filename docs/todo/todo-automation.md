...
**Refined (2026-06-01, real-world review):**
  - **Vote clamped at both ends (revised 2026-06-01)** — automation never hard-blocks **and never casts a positive vote on a PR with a MUST-FIX**. `changes-requested`/`rejected` → **`NoVote` (0)**, not `ApprovedWithSuggestions`. The earlier "clamp to `ApprovedWithSuggestions` ceiling" was wrong: `ApprovedWithSuggestions` (+5) is a *positive approval* that feeds branch-policy minimum-approvals + auto-complete, so the bot was approving (and helping merge) PRs it had flagged MUST-FIX (observed in a real consumer review — MUST-FIX posted, voted +5, attributed to the author + a required reviewer group). `NoVote` leaves both the approving and blocking decisions to a human; the summary states the bot is **not approving** and flags the MUST-FIX. Also: the `dx-pr-reviewer` agent now derives verdict mechanically (any MUST-FIX ⇒ "Changes requested"), and the pipeline template warns to run the bot under a dedicated service-account PAT. See `post-findings.md` step 6 table (Automation column).
  - **Idempotent posting** — `post-findings.md` step 4 now lists existing threads first and skips findings already anchored to a `(filePath, startLine)` (and the `[AI Review]` summary). Fixes duplicate-thread spam on re-run and the crash-before-vote gap (threads posted, vote not cast → next `vote==0` sweep re-selects the PR → previously re-posted everything).
...
**Refined (2026-06-01, real-world review):** added a budget on top of the confidence bar to bound bot↔human arguing — the convergence fix for "human dismisses AI feedback":
  - **Disagree = one rebuttal, then escalate.** The numeric confidence bar is **retained** (`DISAGREE_MIN` from `DX_DISAGREE_CONFIDENCE` → config → 80) — an explicit, auditable floor for whether a pushback is worth sending. New: a per-thread budget `DX_MAX_DISAGREE_REPLIES` (default 1). The bot posts a substantive pushback only when **confidence ≥ bar AND evidence is cited AND `posted_pushbacks` < budget**, decided from the **live thread comments** (not the session file — CI checks out fresh each run). Once the budget is spent — the bot already rebutted and the human replied again ("already covered", a dismissal, a counter-argument) — or below the bar / no evidence, it posts a `needs-human-input` @-mention instead of arguing the same point twice. The bar decides IF to push back; the budget stops REPEAT arguing. Verify: `grep -n 'DX_MAX_DISAGREE_REPLIES' …/dx-pr-answer/SKILL.md`; `grep -n 'DISAGREE_MIN' …/dx-pr-answer/SKILL.md`.
...
- `node --check` passes for every lib + handler; importing `wi-router.mjs` and `pr-router.mjs` resolves (handler is a function) both from the repo tree and from an extracted deploy zip. Superseded the flat-zip layout — see #211.
**Approach (implemented):** Recovered the 5 libs from `1800b25^` via `git show`. Confirmed the dependency closure is self-contained (`dedupe`/`dlq`/`rate-limiter` → `aws-sig` → `retry` → Node `crypto`; no dependency on any other deleted file), so none of the old-agent code comes back. Placed them in `data/lambda/lib/` (with the Lambda code) rather than resurrecting `data/agents/lib/`, and pointed `deploy.sh` at `lib/`. The handlers' `./dedupe.js` imports were left unchanged at the time, with `deploy.sh` flattening the libs next to the handlers in the zip — which meant the handlers could not be imported from the repo tree at all. #211 changed both sides to `lib/`. Cross-ref #7 (the cleanup), #150, #211.

## Azure-native trigger parity for the remaining agents

**Added:** 2026-09-19
**Problem:** Azure-native Service Hooks are the default and recommended trigger path (no AWS at all), but only four pipelines ship a `resources.webhooks` listener: `ado-cli-simple.yml`, `ado-cli-bug-fix.yml`, `ado-cli-dor.yml`, `ado-cli-hub.yml`. The six tag-routed work-item agents (DoD, DoD fixer, QA, DevAgent, DOCAgent, Estimation) can only be fired Azure-natively *through* the hub router or by a manual run — a single-repo project cannot point a Service Hook straight at them. PR Answerer is worse: it is genuinely Lambda-only, because the identity / self-comment / bot-loop gates live in `pr-router.mjs` and have no pipeline equivalent. So "Lambda is optional" is true at the project level but not yet per agent.
**Scope:** `plugins/dx-automation/data/pipelines/cli/ado-cli-{dod,dod-fix,qa,dev-agent,doc-agent,estimation}.yml` (add a listener each, mirroring `ado-cli-dor.yml`: `resources.webhooks` + webhook-id fallback for `workItemId` + revision-based dedup); `ado-cli-pr-answer.yml` (port the PR-Router gates, or lean on the existing `pr-answer-gate.sh` cron sweep and retire the Lambda route); `plugins/dx-automation/skills/auto-webhooks/SKILL.md` (per-agent hook + service-connection setup); `plugins/dx-automation/data/infra.template.json` (`webhooks` entries).
**Done-when:**
- `grep -l 'webhooks:' plugins/dx-automation/data/pipelines/cli/ado-cli-*.yml` lists every comment-triggerable agent, not just simple / bug-fix / dor / hub.
- The per-pipeline trigger table in `plugins/dx-automation/README.md` has no ❌ rows left, or the remaining ones state a reason that is a design decision rather than unfinished work.
- `bash scripts/validate-structure.sh` → PASS.
