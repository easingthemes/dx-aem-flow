...
| dx-bug-verify | `/dx-bug-verify` | `<bug-id> [before\|after\|qa]` | Reproduce bug via Playwright. `before`: confirm bug exists. `after`: verify fix locally. `qa`: verify fix on QA post-merge. | `verification.md` / `-local.md` / `-qa.md` |
...
| dx-simple | `/dx-simple` | `<work-item-id>` | Apply a small AEM change (a11y label, color token, spacing, copy, css-class, icon) by splitting work into authoring (AEM MCP JCR writes) and code (file edits → PR) paths. Story SHOULD include a `\`\`\`simple` fenced block — only `page-url` is required; `element`, `what`, `why` (plus optional `brand`/`platform`/`scope`/`activate`) are inferred from the story prose if omitted. Enforces 9 confidence gates and ≤5 files / ≤50 lines / ≤10 JCR writes. Reuses `aem-file-resolver`, `aem-inspector`, `aem-page-finder`, `dx-pr-reviewer` agents. Uses `model: sonnet`. Also runs autonomously as SimpleAgent (dx-automation), triggered by a `@kai-simple` comment via an Azure-native Service Hook — no Lambda. **Resumable recovery (TODO #141):** Phase 0 discovers the ticket's branch (anchored id match) and dispatches on committed `resume-state.json`; every phase/gate checkpoints state to the branch; blocked/crashed runs resume when a human replies `@kai-simple` (config token `dx-simple.recovery.*`). Requires `.ai/specs/` tracked in git. | `.ai/specs/<id>-simple/report.md` + ADO comment + PR (if code path) |
